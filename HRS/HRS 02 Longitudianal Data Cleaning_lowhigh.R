...
####preparing the df####
##define sample size; baseline 2010
sum(df$inw2010)
df <- df[df$inw2010 == 1,]
...
selected_columns <- grep("^cogfunction", names(df), value = TRUE)
...
# exclude participants with dementia at baseline 2010
df <- df[df$cogfunction2010_dementia != 1, ]
...
vars <- c("health", "stress", "financial", "warmth")

missing_childhood <- data.frame(
  variable = vars,
  missing_prop = sapply(df[vars], function(x) round(mean(is.na(x))*100, 2))
) %>%
  arrange(desc(missing_prop))

print(missing_childhood)

##################################################
#missingness check - baseline 2010
...
table(df[df$education_quan == "2",]$years_edu)
table(df$education_quan)

table(df[df$education_p_quan == "2",]$highest_education_parent)
table(df$education_p_quan)

#covariates imputation - baseline 2010
...
write.csv(df,"D:\\R project\\Educational Mobility and dementia\\Analysis\\HRS\\imputed_hrs_long_lowhigh_2010baseline.csv")